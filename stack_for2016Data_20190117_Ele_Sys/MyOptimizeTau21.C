...
void MyOptimizeTau21(){
  ...
  for(int i = 0; i<massPoints.size(); i++){
   ...
   if(isMuChannel)decorateGraph(myGraph, "#tau_{21}", "#epsilon_{Signal}/#epsilon_{Bkg}", "optimization of #tau_{21} :  #mu-channel", 0.8, 2.0, color);
   if(isEleChannel)decorateGraph(myGraph, "#tau_{21}", "#epsilon_{Signal}/#epsilon_{Bkg}", "optimization of #tau_{21} :  e-channel", 0.8, 2.0, color);
   if(i==0) myGraph->Draw("ALP");
   else myGraph->Draw("LPsame");
   leg->AddEntry(myGraph, TString(massLables[i]),"PL");
  }
  leg->Draw();
  if(isMuChannel)can->SaveAs("tau21_mu.png");
  if(isEleChannel)can->SaveAs("tau21_ele.png");
}
